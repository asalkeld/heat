...
class AutoScalingPolicy(common.HeatTestCase):
    params = {'KeyName': 'test', 'ImageId': 'foo'}

    def setUp(self):
        super(AutoScalingPolicy, self).setUp()
    ...
    def test_scaling_policy_not_alarm_state(self):
        ...
        with mock.patch.object(pol, '_cooldown_inprogress',
            ...
            self.assertItemsEqual(dont_call.call_args_list, [])
...
class CooldownMixin(common.HeatTestCase):
    params = {'KeyName': 'test', 'ImageId': 'foo'}

    def setUp(self):
        super(CooldownMixin, self).setUp()
        cfg.CONF.set_default('heat_waitcondition_server_url',
                             'http://server.test:8000/v1/waitcondition')
        self.stub_keystoneclient()
